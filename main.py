...
parser.add_argument('--dataset', default = 'mnist', help='only mnist supported so far')
...
paramsG = list(gen.parameters()) + list(qB.parameters())
paramsD = list(disF.parameters()) + list(disB.parameters()) 
gOpt = op.Adam(paramsG, lr=hp.lr, betas=(hp.b1, hp.b2))
dOpt = op.Adam(paramsD, lr=hp.lr,betas=(hp.b1, hp.b2))

lossD = nn.BCELoss().cuda() #loss function for Discriminator
lossGDisc = nn.CrossEntropyLoss() # loss function for Gen, Disc case
...
for epoch in range(hp.niter):
	...
	for itr, (imgsReal, _) in enumerate(train_loader):
				...
				lossReal = lossD(actualDReal, expectedDReal)


				imgGFake = gen(z)
				feFakeD = disF(imgGFake.detach())
				actualDFake = disB(feFakeD)
				expectedDFake = Variable(torch.FloatTensor(hp.batchSize,1).fill_(random.uniform(0.0, 0.1)), requires_grad=False)
				lossFake = lossD(actualDFake, expectedDFake)
				...
				gOpt.zero_grad()
				feFakeG = disF(imgGFake)
				actualGFake = disB(feFakeG)
				expectedGFake = Variable(torch.FloatTensor(hp.batchSize,1).fill_(random.uniform(0.9, 1.0)), requires_grad=False)
				gLossTrick = lossD(actualGFake, expectedGFake)

				#Mutual information loss.

				#Discrete variable loss
				qResult, mu, var = qB(feFakeG)
				minAct = cDisc.squeeze()
				#x= torch.mean(torch.softmax(qResult, dim=1).log()* torch.max(minAct,1)[1])
				minAct = minAct.type(torch.LongTensor)
				mInfLossDis = lossGDisc(qResult, torch.max(minAct,1)[1])
				...
				if (itr % 50) == 0:
					print("Epoch: [%2d] Iter: [%2d] D_loss: %.8f, G_loss: %.8f \ G_Loss_QLoss: %.8f, DiscLoss: %.8f, G_Loss_GenLossonDisc: %.8f" 
						% (epoch, itr, dLoss.item(), gLoss.item(), qLoss.item(), mInfLossDis.item(), gLossTrick.item()))
				...
				if (itr % hp.saveInterval) == 0:
					...
					for itr2, (imgsRealTest, _) in enumerate(test_loader):
						if itr2 < 1: 
							...
							lossRealTest = lossD(actualRealTest, expectedRealTest)
